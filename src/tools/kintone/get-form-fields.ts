...
const fieldPropertySchema = z.object({
  ...
  unitPosition: z.enum(["BEFORE", "AFTER"]).optional().describe("Unit position"),
  ...
  protocol: z.enum(["WEB", "CALL", "MAIL"]).optional().describe("Link protocol"),
  lookup: z.object({
    relatedApp: z.object({
      app: z.string(),
      code: z.string(),
  ...
  }).optional().describe("Lookup settings"),
